...
import com.huatu.springboot.dubbo.annotation.EnableDubboConfiguration;
...
public class DubboConsumerAutoConfiguration {
    ...
    private <T> ReferenceBean<T> getConsumerBean(Class<T> interfaceClazz, DubboConsumer dubboConsumer)
        ...
        if (group == null || "".equals(group)) {
            group = this.properties.getConsumer().getGroup();
        }
        if (group != null && !"".equals(group)) {
        ...
        if (version == null || "".equals(version)) {
            version = this.properties.getConsumer().getVersion();
        }
        if (version != null && !"".equals(version)) {
